...
module "backend_server" {
  ...
  user_data = <<-EOF
    ...
    docker compose -f /home/ubuntu/recall/backend/docker-compose.yml start
  EOF
}


module "frontend_launch_template" {
  source = "./modules/frontend_launch_template"
...
module "ssl_cert" {
  ...
  project_name = local.config.deployend.project_name_lowercase
}
...
module "domain_mgt" {
  ...
  internal_alb_zone_id      = module.internal_alb.internal_alb_zone_id
  vpc_id                    = module.vpc.vpc_id
  project_name              = local.config.deployend.project_name_lowercase
}
...
module "cloudfront" {
  ...
  origin_domain_name  = module.frontend_alb.alb_dns_name
  aliases             = ["${local.config.deployend.project_name_lowercase}.${local.config.deployend.domain_name}"]
}